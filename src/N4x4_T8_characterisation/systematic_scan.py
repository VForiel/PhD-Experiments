...
def run(
    ...
    Notes
    -----
    - This function controls hardware. Use with care and ensure the
      instrument environment is prepared (laser off / filter wheel set)
      if required.
    - The function mirrors the behaviour of `Arch.characterize` but
      intentionally does not perform fits or plotting.
    """

    # Build phase vector
    phases = np.linspace(phase_range[0], phase_range[1], phase_samples)

    # All possible input combinations: 1-input, 2-inputs, ..., all
    ...
    for active_inputs in input_combinations:
        ...
        # Ensure inputs: turn off all inputs then enable selected ones
        dm.off()
        # DM.flat accepts a list of input indices to keep on
        try:
            dm.flat(list(active_inputs))
        except Exception:
            # If DM.flat does not accept arguments, call without and assume
            # inputs are controlled elsewhere. We still proceed.
            try:
                dm.flat()
            except Exception:
                # Last resort: no-op
